...
export const namespaceHelp = (ns, path) => {
    const sections = [];
    if (ns.description)
        sections.push(ns.description);
    sections.push(`Usage: ${path.join(' ')} <command> [options]`);
    const children = ns.operations.map((c) => api.of(c));
    const commands = children.map((child) => [child.name, child.description ?? '']);
...
export const operationHelp = (op, path) => {
    ...
    if (args.length > 0)
        sections.push(section('Arguments', args));
...
const defaultNote = (field) => field.default === undefined ? undefined : `default: ${JSON.stringify(field.default)}`;
const requiredNote = (field) => (field.required ? 'required' : '');
const annotate = (text, ...notes) => {
    const kept = notes.filter((n) => n !== undefined && n !== '');
