...
import { t } from '../build.ts'
import * as bind from './bind.ts'
...
const cli = (property: Property): { value: unknown } | undefined =>
  property.shape.list
    ? { value: [] }
...
test('a default or a fallback is taken as given, never validated', async () => {
  ...
  const odd = t.object({ n: t.number().default('nope' as unknown as number) })
...
test('an object read once binds the same as one read per call', async () => {
  ...
  expect(await bind.to(prepared, { title: 'x' }, { fallback: cli })).toEqual(
    await bind.to(schema, { title: 'x' }, { fallback: cli }),
  )
})
