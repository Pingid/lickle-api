...
import { findChild, operations, walk } from './visit.ts'
...
test('a tree that refers to itself still has a type', () => {
  ...
  const tree: Namespace = t.ns({ name: 'todo', operations: [t.op({ name: 'echo', handle: async () => {} })] })
  const needs: Namespace.Cx<typeof tree> = undefined
