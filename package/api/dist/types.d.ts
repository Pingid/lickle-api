...
export declare namespace Api {
    ...
    type Wrap<T extends AnyOperation | AnyNamespace | Api> = T extends AnyOperation | AnyNamespace ? Api<T> : T;
}
export type AnyOperation = Operation<any, any, any>;
export interface Operation<N extends string, I extends Input = Input, O extends Output = Output> extends Operation.Schema<N, I, O>, Operation.Handler<Input.Infer<I>, Output.Infer<O>> {
}
export declare namespace Operation {
    interface Schema<N extends string, I, O> {
        name: N;
        description?: string;
        in: I;
        out: O;
...
export interface Namespace<N extends string = string, O extends (Api<any> | AnyOperation | AnyNamespace)[] = (Api<any> | AnyOperation | AnyNamespace)[]> {
    _type?: 'namespace';
    name: N;
    description?: string;
    operations: O;
...
export declare namespace Namespace {
    type DecendTree<T, Sep extends string = '/', N extends string = ''> = T extends Api<infer I> ? DecendTree<I, Sep, N> : T extends AnyOperation ? {
        [ky in `${N}${Sep}${T['name']}`]: T;
    } : T extends AnyNamespace ? DecendTree<T['operations'][number], Sep, `${N}${Sep}${T['name']}`> : never;
...
export declare namespace Output {
    ...
    type Infer<S extends Output> = S['returns'] extends 'async-iter' ? AsyncIterable<Schema.InferOutput<S>> : S['returns'] extends 'sync' ? Schema.InferOutput<S> : Promise<Schema.InferOutput<S>>;
}
