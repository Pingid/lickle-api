...
const args = (given: Record<string, unknown>) => bindArgs(bind.inputs(op.in), given)
...
test('a default fills a gap; nothing else does', async () => {
  ...
  await expect(args({ title: 'x', mode: 'fast' })).rejects.toThrow(
    `missing required argument 'tags'; missing required argument 'done'`,
  )
})
...
test('an unknown argument names the ones that exist, so the model can correct itself', async () => {
  await expect(args({ ...whole, nope: 1 })).rejects.toThrow(
    `unknown argument 'nope'; expected 'title', 'tags', 'done', 'times', 'note' and 'mode'`,
  )
...
test('every problem comes back at once, so one turn can fix them all', async () => {
  await expect(args({ title: 5, nope: 1 })).rejects.toThrow(
    /unknown argument 'nope'.*invalid value for 'title'.*missing required argument 'tags'/s,
  )
})
...
test('a foreign input validates whole, and each issue names the argument that carried it', async () => {
  ...
  await expect(bindArgs(inputs, { to: 'a@b.com', count: '2' })).rejects.toThrow(
