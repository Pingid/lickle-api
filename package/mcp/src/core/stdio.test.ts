...
import { JSONRPC_VERSION, PARSE_ERROR } from './wire.ts'

/** Answers every request with its own id, and nothing to a notification. */
const echo: Dispatch = async (message) => {
...
const drive = async (chunks: (string | Uint8Array)[], dispatch: Dispatch = echo) => {
  const out: string[] = []
  ...
  for (const frame of out) expect(frame.endsWith('\n')).toBe(true)
  return out.map((frame) => JSON.parse(frame))
}

const request = (id: number, method = 'tools/list') => JSON.stringify({ jsonrpc: JSONRPC_VERSION, id, method })
...
test('a notification a call emits is written before that call answers', async () => {
  const talking: Dispatch = async (message, emit) => {
    emit?.({ jsonrpc: JSONRPC_VERSION, method: 'notifications/progress', params: { progress: 1 } })
    return { jsonrpc: JSONRPC_VERSION, id: (message as { id: number }).id, result: {} }
  }
  const got = await drive([`${request(1)}\n`], talking)
