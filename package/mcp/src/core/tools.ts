...
import { api, bind, is, meta, operations, schema as js, type Located } from '@lickle/api/util'

import type { McpMeta } from '../meta.ts'
import { toolNames, type NamePolicy } from './names.ts'
...
export const nodeMeta = (node: object): McpMeta => meta.of<McpMeta>(node, 'mcp') ?? {}
 ...
 */
export const isHidden = (node: Operation.Any | Namespace): boolean => nodeMeta(node).hidden === true
 ...
 */
export const tools = (program: Api.Program, opts: ToolsOpts = {}): ToolEntry[] => {
  const root = api.of(program)
  const warn = opts.onWarn ?? ((message: string) => void console.error(message))

  const found: Located<Operation.Any>[] = is.operation(root)
    ? isHidden(root)
      ? []
  ...
  const named = toolNames(
    found.map((located) => nodeMeta(located.node).name?.split(' ') ?? located.path),
    opts.naming,
  )

  return found.map((located, index): ToolEntry => {
    const { name, wanted } = named[index]!
    if (wanted !== undefined) warn(`tool name '${wanted}' is taken; '${located.path.join(' ')}' is '${name}'`)
    ...
    return {
      tool: {
        ...
        inputSchema: js.json(op.in) as Tool['inputSchema'],
        // A streaming operation declares the schema of one item, so what a call
        // answers with — all of them at once — is a list of that.
        ...(structured ? { outputSchema: (stream ? { type: 'array', items: out } : out) as Tool['outputSchema'] } : {}),
