 ...
 */
const absent = (args: Record<string, unknown>): Record<string, unknown> =>
  Object.fromEntries(Object.entries(args).filter(([, value]) => value !== null && value !== undefined))
 ...
 */
const say = (problem: bind.Problem, expected: string[]): string => {
  const at = (key: string, path: string) => (path === '' ? key : `${key}.${path}`)
  switch (problem.kind) {
    case 'missing':
