...
import { JSONRPC_VERSION } from './core/wire.js';
import { mcp } from './serve.js';
const echo = t.op({
    name: 'echo',
...
const build = (input, out) => {
    const tree = t.ns({
        ...
        operations: [echo, mcp(() => tree, { io: { input, write: (c) => void out.push(c) } })],
    });
    return tree;
};
const serveOf = (tree) => {
    const found = tree.operations.map((o) => api.of(o)).find((o) => o.name === 'mcp');
    if (found === undefined || !('handle' in found))
        throw new Error('no mcp operation');
    return found;
};
const lines = async function* (...chunks) {
...
test('it serves the tree it sits in, over its own stream', async () => {
    ...
    const names = JSON.parse(out[0]).result.tools.map((x) => x.name);
