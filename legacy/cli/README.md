...
Run a `@lickle/api-legacy` operation as a command-line program.
...
```sh
pnpm add @lickle/cli-legacy
```

`@lickle/cli-legacy/cmd` re-exports `@lickle/api-legacy`, so this one package is
enough to write a CLI. Depend on `@lickle/api-legacy` directly when a package
...
```ts
// todo.ts
import { cli, run } from '@lickle/cli-legacy'
import { bool, cmd, field, list, num, string, type Namespace } from '@lickle/cli-legacy/cmd'
...
```ts
import { InputError } from '@lickle/cli-legacy/cmd'
