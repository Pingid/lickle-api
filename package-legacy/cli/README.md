...
Run a `@lickle/legacy-api` operation as a command-line program.
...
```sh
pnpm add @lickle/legacy-cli
```

`@lickle/legacy-cli/cmd` re-exports `@lickle/legacy-api`, so this one package is
enough to write a CLI. Depend on `@lickle/legacy-api` directly when a package
...
```ts
// todo.ts
import { cli, run } from '@lickle/legacy-cli'
import { bool, cmd, field, list, num, string, type Namespace } from '@lickle/legacy-cli/cmd'
...
```ts
import { InputError } from '@lickle/legacy-cli/cmd'
